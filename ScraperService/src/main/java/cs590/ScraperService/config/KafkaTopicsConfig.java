...
import cs590.ScraperService.messaging.Topics;
 ...
 * publishes to — ScraperService owns {@code scraped-job}. Replication factor must be 3 on Serverless.
 */
@Configuration
public class KafkaTopicsConfig {

    @Bean
    NewTopic scrapedJobTopic() {
        return TopicBuilder.name(Topics.SCRAPED_JOB).partitions(6).replicas(3).build();
    }
}
